...
class GroupStatTestResults(object):
    ...
    def filterFeatures(self, signLevelFilter,
        ...
        for row in self.data:
            ...
            # p-value filter
            if signLevelFilter is not None and pValue > signLevelFilter:
                continue
            ...
            if parentSeqFilter == 'maximum':
                ...
                bPassParentSeqFilter = (min(parentSeqCountGroup1) >= parentGroup1Filter and min(
                    parentSeqCountGroup2) >= parentGroup2Filter)
            elif parentSeqFilter == 'independent, maximum':
                bPassParentSeqFilter = (max(parentSeqCountGroup1) >= parentGroup1Filter and max(
                    parentSeqCountGroup2) >= parentGroup2Filter)
            else:
            ...
            effectSize1 = 0
            if effectSizeMeasure1 is not None:
            ...
            effectSize2 = 0
            if effectSizeMeasure2 is not None:
            ...
            if effectSizeMeasure1 is None and effectSizeMeasure2 is None:
                self.activeData.append(row)
            elif effectSizeMeasure1 is not None and effectSizeMeasure2 is None:
                if effectSize1 >= minEffectSize1:
                    self.activeData.append(row)
            elif effectSizeMeasure1 is None and effectSizeMeasure2 is not None:
    ...
    def run(self, statTest, testType, confIntervMethod, coverage, profile, progress=None):
        ...
        if statTest.bSingleFeatureInterface:
            ...
            for feature in profile.getFeatures():
                ...
                results = statTest.run(seqGroup1, seqGroup2, parentSeqGroup1, parentSeqGroup2, confIntervMethod,
                                       coverage)
            ...
            pValuesOneSided, pValuesTwoSided, lowerCIs, upperCIs, effectSizes, notes = statTest.runAll(seqsGroup1,
                                                                                                       seqsGroup2,
                                                                                                       parentSeqsGroup1,
                                                                                                       parentSeqsGroup2,
                                                                                                       confIntervMethod,
            ...
            elif progress is not None and progress.wasCanceled():
                self.results.data = []
                return
