...
class MainWindow(QtWidgets.QMainWindow):
	def __init__(self, preferences, parent=None):
		...
		self.refreshIcon.addPixmap(QtGui.QPixmap("icons:refresh.png"), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
		...
		self.addDockWidget(QtCore.Qt.DockWidgetArea.RightDockWidgetArea, self.groupLegendDlg)
		...
		self.addDockWidget(QtCore.Qt.DockWidgetArea.BottomDockWidgetArea, self.metadataDlg)
		...
		self.ui.mnuFileExit.triggered.connect(self.close)
		...
		self.ui.cboProfileLevel.textActivated.connect(self.profileLevelChanged)
		self.ui.cboParentalLevel.textActivated.connect(self.parentLevelChanged)
		self.ui.cboUnclassified.textActivated.connect(self.unclassifiedTreatmentChanged)
		...
		self.restoreState(windowSettings.value("MainWindow/State", QtCore.QByteArray()))
		bRestoredState = self.restoreGeometry(windowSettings.value("MainWindow/Geometry", QtCore.QByteArray()))
	...
	def setupSampleWidgets(self):
		...
		self.addDockWidget(QtCore.Qt.DockWidgetArea.BottomDockWidgetArea, self.sampleTable)
		...
		self.ui.cboSample1.textActivated.connect(self.sampleHierarchicalLevelsChanged)
		self.ui.cboSample2.textActivated.connect(self.sampleHierarchicalLevelsChanged)
		...
		self.ui.cboSampleStatTests.textActivated.connect(self.sampleRunTest)
		self.ui.cboSampleSignTestType.textActivated.connect(self.sampleRunTest)
		self.ui.cboSampleConfIntervMethods.textActivated.connect(self.sampleRunTest)
		self.ui.cboSampleNominalCoverage.textActivated.connect(self.sampleRunTest)
		self.ui.cboSampleMultCompMethod.textActivated.connect(self.sampleMultCompCorrectionChanged)
		...
		self.ui.cboSampleSeqFilter.textActivated.connect(self.sampleSeqFilterChanged)
		...
		self.ui.cboSampleParentSeqFilter.textActivated.connect(self.sampleParentSeqFilterChanged)
		...
		self.ui.cboSampleEffectSizeMeasure1.textActivated.connect(self.sampleChangeEffectSizeMeasure)
		self.ui.cboSampleEffectSizeMeasure2.textActivated.connect(self.sampleChangeEffectSizeMeasure)
		...
		self.ui.cboSamplePlots.textActivated.connect(self.samplePlotUpdate)
		self.ui.btnSampleConfigurePlot.clicked.connect(self.samplePlotConfigure)
		self.ui.cboSampleHighlightHierarchy.textActivated.connect(self.sampleHighlightHierarchyChanged)
		self.ui.cboSampleHighlightFeature.textActivated.connect(self.sampleHighlightFeatureChanged)
	...
	def setupGroupWidgets(self):
		...
		self.addDockWidget(QtCore.Qt.DockWidgetArea.BottomDockWidgetArea, self.groupTable)
		...
		# connect profile widget signals to slots
		self.ui.cboGroup1.textActivated.connect(self.groupHierarchicalLevelsChanged)
		self.ui.cboGroup2.textActivated.connect(self.groupHierarchicalLevelsChanged)
		self.ui.btnGroup1Colour.clicked.connect(self.group1ColourDlg)
		self.ui.btnGroup2Colour.clicked.connect(self.group2ColourDlg)

		# connect statistical test widget signals to slots
		self.ui.cboGroupStatTests.textActivated.connect(self.groupRunTest)
		self.ui.cboGroupSignTestType.textActivated.connect(self.groupRunTest)
		self.ui.cboGroupConfIntervMethods.textActivated.connect(self.groupRunTest)
		self.ui.cboGroupNominalCoverage.textActivated.connect(self.groupRunTest)
		self.ui.cboGroupMultCompMethod.textActivated.connect(self.groupMultCompCorrectionChanged)
		...
		self.ui.cboGroupSeqFilter.textActivated.connect(self.groupSeqFilterChanged)
		...
		self.ui.cboGroupParentSeqFilter.textActivated.connect(self.groupParentSeqFilterChanged)
		...
		self.ui.cboGroupEffectSizeMeasure1.textActivated.connect(self.groupChangeEffectSizeMeasure)
		self.ui.cboGroupEffectSizeMeasure2.textActivated.connect(self.groupChangeEffectSizeMeasure)
		...
		# connect statistical plot page widget signals to slots
		self.ui.cboGroupPlots.textActivated.connect(self.groupPlotUpdate)
		self.ui.btnGroupConfigurePlot.clicked.connect(self.groupPlotConfigure)
		self.ui.cboGroupHighlightHierarchy.textActivated.connect(self.groupHighlightHierarchyChanged)
		self.ui.cboGroupHighlightFeature.textActivated.connect(self.groupHighlightFeatureChanged)
	...
	def setupMultiGroupWidgets(self):
		...
		self.addDockWidget(QtCore.Qt.DockWidgetArea.BottomDockWidgetArea, self.multiGroupTable)
		...
		# connect statistical test widget signals to slots
		self.ui.cboMultiGroupStatTests.textActivated.connect(self.multiGroupRunTest)
		self.ui.cboMultiGroupMultCompMethod.textActivated.connect(self.multiGroupMultCompCorrectionChanged)
		self.ui.btnMultiGroupMultCompCorrectionInfo.clicked.connect(self.multiGroupMultCompCorrectionInfo)
		self.ui.cboPostHocTest.textActivated.connect(self.multiGroupPlotUpdate)
		self.ui.cboMultiGroupNominalCoverage.textActivated.connect(self.multiGroupPlotUpdate)
		...
		# connect statistical plot page widget signals to slots
		self.ui.cboMultiGroupPlots.textActivated.connect(self.multiGroupPlotUpdate)
		self.ui.btnMultiGroupConfigurePlot.clicked.connect(self.multiGroupPlotConfigure)
		self.ui.cboMultiGroupHighlightHierarchy.textActivated.connect(self.multiGroupHighlightHierarchyChanged)
		self.ui.cboMultiGroupHighlightFeature.textActivated.connect(self.multiGroupHighlightFeatureChanged)
	...
	def updateSideBarTabIcon(self, tab, arrowButton):
		icon = QtGui.QIcon()
		if tab.isVisible():
			icon.addPixmap(QtGui.QPixmap("icons:downArrow.png"), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
		else:
			icon.addPixmap(QtGui.QPixmap("icons:rightArrow.png"), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
	...
	def prefrencesDlg(self):
		...
		if preferencesDlg.exec() == QtWidgets.QDialog.DialogCode.Accepted:
	...
	def multiGroupPlotUpdate(self):
		...
		if self.multiGroupStatsTest.results.data != []:
			if self.multiGroupPlot.checkFlags().bRunPostHocTest:
				...
				postHocTest = self.postHocTestDict[self.ui.cboPostHocTest.currentText()]
	...
	def sample1ColourDlg(self):
		colour = QtWidgets.QColorDialog.getColor(self.preferences['Sample 1 colour'], self, 'Colour for sample 1')
	...
	def sample2ColourDlg(self):
		colour = QtWidgets.QColorDialog.getColor(self.preferences['Sample 2 colour'], self, 'Colour for sample 2')
	...
	def group1ColourDlg(self):
		colour = QtWidgets.QColorDialog.getColor(self.preferences['Group colours'][self.groupProfile.groupName1], self, 'Colour for group 1')
	...
	def group2ColourDlg(self):
		colour = QtWidgets.QColorDialog.getColor(self.preferences['Group colours'][self.groupProfile.groupName2], self, 'Colour for group 2')
	...
	def loadProfile(self):
		...
		if loadDataDlg.exec() == QtWidgets.QDialog.DialogCode.Accepted:
			...
			try:
				...
				if errMsg != None:
					QtWidgets.QMessageBox.warning(self, 'Error reading profile file', errMsg)
					return
			except:
				QtWidgets.QMessageBox.warning(self, 'Error reading profile file', 'Unknown parsing error.')
				return
			...
			if metadataFile != '':
				try:
					...
					QtWidgets.QMessageBox.warning(self, 'Error reading metadata file', 'Unknown parsing error.')
					return
	...
	def parentLevelChanged(self):
		...
		if parentDepth >= profileDepth:
			QtWidgets.QMessageBox.warning(self, 'Invalid profile', 'The parent level must be higher in the hierarchy than the profile level.')
			self.ui.cboParentalLevel.setCurrentIndex(0)
	...
	def profileLevelChanged(self):
		...
		if profileDepth <= parentDepth:
			QtWidgets.QMessageBox.warning(self, 'Invalid profile', 'The profile level must be deeper in the hierarchy than the parent level.')
	...
	def groupFeaturesTableUpdate(self):
		...
		self.groupFeatureTable.sort(0, QtCore.Qt.SortOrder.AscendingOrder)  # start with features in alphabetical order
	...
	def multiGroupFeaturesTableUpdate(self):
		...
		self.multiGroupFeatureTable.sort(0, QtCore.Qt.SortOrder.AscendingOrder)  # start with features in alphabetical order
		...
		self.tableMultiGroupSelectionModel = QtCore.QItemSelectionModel(self.multiGroupFeatureTable, self.ui.tableMultiGroupFeatures)
	...
	def groupTableFeatureChanged(self, selected, deselected):
		...
		self.preferences['Selected group feature'] = str(selectedRow.data(QtCore.Qt.ItemDataRole.DisplayRole))
		self.groupPlotUpdate()

	def multiGroupSideTableFeatureChanged(self, selected, deselected):
		selectedRow = selected.indexes()[0]
		self.preferences['Selected multiple group feature'] = str(selectedRow.data(QtCore.Qt.ItemDataRole.DisplayRole))
		self.multiGroupPlotUpdate()
	...
	def groupRunTest(self):
		...
		if groupName1 != '' and groupName2 != '':
			...
			# show progress of test
			progress = QtWidgets.QProgressDialog('Running two-group statistical test...', 'Cancel', 0, len(self.groupProfile.getFeatures()) + 1, self)
			progress.setWindowTitle('Progress')
			progress.setWindowModality(QtCore.Qt.WindowModality.WindowModal)
			progress.setVisible(True)
	...
	def multiGroupRunTest(self):
		...
		if len(self.multiGroupProfile.activeGroupNames) >= 2:
			# show progress of test
			progress = QtWidgets.QProgressDialog('Running multiple groups statistical test...', 'Cancel', 0, len(self.multiGroupProfile.getFeatures()) + 1, self)
			progress.setWindowTitle('Progress')
			progress.setWindowModality(QtCore.Qt.WindowModality.WindowModal)
			progress.setVisible(True)
	...
	def sampleMultCompCorrectionInfo(self):
		if self.sampleStatsTest.results.multCompCorrection != None:
			...
			QtWidgets.QMessageBox.information(self, 'Run test', 'Run hypothesis test first.', QtWidgets.QMessageBox.StandardButton.Ok)
	...
	def sampleSelectFeaturesDlg(self):
		...
		if selectFeatureDialog.exec() == QtWidgets.QDialog.DialogCode.Accepted:
	...
	def saveImageDlg(self):
		...
		if stackedWidget.currentIndex() == 2:
			...
			QtWidgets.QMessageBox.information(self, 'Select plot', 'A plot tab must be active to save a plot.', QtWidgets.QMessageBox.StandardButton.Ok)
			return
		...
		f = QtWidgets.QFileDialog.getSaveFileName(self, 'Save plot...', self.preferences['Last directory'],
								...
								'Scalable Vector Graphics (*.svg)')[0]

		if f != '':
			self.preferences['Last directory'] = f[0:f.rfind('/')]
			try:
				if f[len(f) - 3:len(f)] == 'png' or f[len(f) - 3:len(f)] == 'PNG':
					dpi, ok = QtWidgets.QInputDialog.getInteger(self, 'Desired resolution', 'Enter desired resolution (DPI) of image:', 300)
					if ok:
						plotToSave.save(str(f), dpi)
				else:
					plotToSave.save(str(f))
			except IOError:
					QtWidgets.QMessageBox.information(self, 'Failed to save image', 'Write permission for file denied.', QtWidgets.QMessageBox.StandardButton.Ok)
	...
	def updateStatusBar(self):
		# No profile loaded yet, or the last load failed validation (read() returned
		# None), so profileTree is None or empty. There is nothing to report and the
		# per-tab branches below would dereference a missing profile, so bail out.
		if self.profileTree == None or self.profileTree.numSamples() < 2:
...
def main():
	...
	preferences['Pseudocount'] = settings.value('Preferences/Pseudocount', 0.5, type=float)
	preferences['Replicates'] = settings.value('Preferences/Replicates', 1000, type=int)
	preferences['Truncate feature names'] = settings.value('Preferences/Truncate feature names', True, type=bool)
	preferences['Length of truncated feature names'] = settings.value('Preferences/Length of truncated feature names', 50, type=int)
	...
	preferences['Minimum reported p-value exponent'] = settings.value('Preferences/Minimum reported p-value exponent', -15, type=float)
