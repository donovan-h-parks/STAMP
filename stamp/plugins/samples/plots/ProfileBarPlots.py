...
class ProfileBarPlots(AbstractSamplePlotPlugin):
    ...
    def __init__(self, preferences, parent=None):
        super(ProfileBarPlots, self).__init__(preferences, parent)
        ...
        self.bShowPvalue = self.settings.value(self.name + '/show p-value', True, type=bool)
        self.pValueThreshold = float(self.settings.value(self.name + '/p-value threshold', 0.05))
        self.bOnlyActiveFeatures = self.settings.value(self.name + '/only active features', True, type=bool)
    ...
    def plot(self, profile, statsResults):
        ...
        # Fix: Convert zip to list for sorting in Python 3
        fields = list(zip(field1, field2, parentField1, parentField2, features))
        fields.sort(reverse=True)
        # ... code populating 'fields' ...

        if not fields:
            # Handle the empty case: clear the plot or print a warning
            print("Warning: No features passed the filter. Plot will be empty.")
            self.canvas.clear()  # Or however STAMP clears the canvas
            return
        ...
        if figWidth > 256 or figHeight > 256:
            ...
            self.emptyAxis()
            QtWidgets.QMessageBox.question(self, 'Excessively large plot',
        ...
        # Determine y-axis max for label scaling
        max_val = 0
        if len(field1) > 0 and len(field2) > 0:
            max_val = max(max(field1), max(field2))

        xLabelBounds, yLabelBounds = self.labelExtents([longestLabel], 8, 90, ['%.0f' % max_val], 8, 0)
        yLabelOffset = 0.3
        padding = 0.15  # inches

        # Avoid division by zero if figWidth is too small
        if figWidth < 0.1: figWidth = 0.1
        ...
        if self.bShowCIs == True:
            rects1 = axesBar.bar(np.arange(len(features)) * colWidth, field1, width=barWidth, color=profile1Colour,
                                 yerr=confInter1, ecolor='black', capsize=self.endCapSize)
            rects2 = axesBar.bar(np.arange(len(features)) * colWidth + barWidth, field2, width=barWidth,
                                 color=profile2Colour, yerr=confInter2, ecolor='black', capsize=self.endCapSize)
        else:
            rects1 = axesBar.bar(np.arange(len(features)) * colWidth, field1, width=barWidth, color=profile1Colour)
            rects2 = axesBar.bar(np.arange(len(features)) * colWidth + barWidth, field2, width=barWidth,
        ...
        axesBar.set_xlim([0, max((len(features) - 1.0) * colWidth + 2 * barWidth + 0.1, 0.1)])
        ...
        # *** Prettify plot
        if self.legendPos != -1 and len(rects1) > 0 and len(rects2) > 0:
            legend = axesBar.legend([rects1[0], rects2[0]], (profile.sampleNames[0], profile.sampleNames[1]),
                                    loc=self.legendPos)
