...
class PostHocPlot(AbstractMultiGroupPlotPlugin):
    ...
    def __init__(self, preferences, parent=None):
        super(PostHocPlot, self).__init__(preferences, parent)
        ...
        self.bShowBarPlot = self.settings.value('multiple group: ' + self.name + '/sequences subplot', True, type=bool)
        self.bShowPValueLabels = self.settings.value('multiple group: ' + self.name + '/p-value labels', True,
                                                     type=bool)
        self.bCustomLimits = self.settings.value('multiple group: ' + self.name + '/use custom limits', False,
                                                 type=bool)
        ...
        self.bShowStdDev = self.settings.value('multiple group: ' + self.name + '/show std. dev.', False, type=bool)
        self.endCapSize = int(self.settings.value('multiple group: ' + self.name + '/end cap size', 0.0))
        self.bPvalueFilter = self.settings.value('multiple group: ' + self.name + '/p-value filter', True, type=bool)

    def mirrorProperties(self, plotToCopy):
        super(PostHocPlot, self).mirrorProperties(plotToCopy)
    ...
    def plot(self, profile, statsResults):
        ...
        if len(statsResults.postHocResults.pValues) > 200:
            ...
            reply = QtWidgets.QMessageBox.question(self, 'Continue?', 'Plots contains ' + str(
                len(statsResults.postHocResults.pValues)) + ' rows. ' +
        ...
        # *** Sort data
        # Use list(zip(...)) for Python 3 compatibility
        data = list(zip(labels, pValues, effectSizes, lowerCIs, upperCIs))
        ...
        if self.imageWidth > 256 or self.imageHeight > 256:
            QtWidgets.QApplication.instance().setOverrideCursor(QtGui.QCursor(QtCore.Qt.ArrowCursor))
            self.emptyAxis()
            reply = QtWidgets.QMessageBox.question(self, 'Excessively large plot',
        ...
        axInitAxis = self.fig.add_axes(
            [xPlotOffsetFigSpace, yPlotOffsetFigSpace, widthPlotFigSpace, heightPlotFigSpace])
        ...
        if self.bShowBarPlot == True:
            ...
            for i in range(0, len(labels)):
                ...
                meanRelFreq1 = statsResults.getDataFromTable(statsResults.postHocResults.feature,
                                                             groupName1 + ': mean rel. freq. (%)')
                meanRelFreq2 = statsResults.getDataFromTable(statsResults.postHocResults.feature,
                                                             groupName2 + ': mean rel. freq. (%)')

                if self.bShowStdDev:
                    stdDev1 = statsResults.getDataFromTable(statsResults.postHocResults.feature,
                                                            groupName1 + ': std. dev. (%)')
                    stdDev2 = statsResults.getDataFromTable(statsResults.postHocResults.feature,
                                                            groupName2 + ': std. dev. (%)')
                ...
                axNumSeq.barh(i + 0.0, meanRelFreq1, height=0.3, xerr=stdDev1, color=colour1, ecolor='black',
                              capsize=endCapSize)
                axNumSeq.barh(i - 0.3, meanRelFreq2, height=0.3, xerr=stdDev2, color=colour2, ecolor='black',
                              capsize=endCapSize)
        ...
        axErrorbar.errorbar(effectSizes, np.arange(len(labels)), xerr=[lowerCIs, upperCIs], fmt='o', ms=markerSize,
                            mfc='black', mec='black', ecolor='black', zorder=10)
...
if __name__ == "__main__":
    ...
    testWindow = TestWindow(PostHocPlot)
    testWindow.show()
    sys.exit(app.exec_())