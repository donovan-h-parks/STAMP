...
class pcaPlot(AbstractMultiGroupPlotPlugin):
    ...
    def __init__(self, preferences, parent=None):
        ...
        # --- FIX: Required flags to prevent AttributeErrors in STAMP.py ---
        ...
        self.bFixedPixelsPerUnitDistance = self.settings.value(
            'multiple group: ' + self.name + '/fixed pixels per unit distance', True, type=bool)
        self.markerSize = int(self.settings.value('multiple group: ' + self.name + '/marker size', 30))
        self.bRotateLabels = self.settings.value('multiple group: ' + self.name + '/rotate pc3 labels', True, type=bool)
        self.bShowPC1vsPC3 = self.settings.value('multiple group: ' + self.name + '/showPC1vsPC3', True, type=bool)
        self.bShowPC3vsPC2 = self.settings.value('multiple group: ' + self.name + '/showPC3vsPC2', True, type=bool)
        self.legendPos = int(self.settings.value('multiple group: ' + self.name + '/legend position', -1))
        self.bUniqueShapes = self.settings.value('multiple group: ' + self.name + '/unique shapes', True, type=bool)
    ...
    def plot(self, profile, statsResults):
        ...
        for i in range(0, len(profile.activeSamplesInGroups)):
            ...
            try:
                axesScatter1.scatter(pc1[start:end], pc2[start:end], s=self.markerSize, zorder=5, c=colours[start:end],
        ...
        if (bPlot2D == False) and self.bShowPC3vsPC2:
            ...
            for i in range(0, len(profile.activeSamplesInGroups)):
                ...
                try:
                    axesScatter2.scatter(pc3[start:end], pc2[start:end], s=self.markerSize, zorder=5,
                                         c=colours[start:end], marker=markers[i % len(markers)])
                except Exception:
        ...
        if (bPlot2D == False) and self.bShowPC1vsPC3:
            ...
            for i in range(0, len(profile.activeSamplesInGroups)):
                ...
                try:
                    axesScatter3.scatter(pc1[start:end], pc3[start:end], s=self.markerSize, zorder=5,
                                         c=colours[start:end], marker=markers[i % len(markers)])
                except Exception:
                    # failed to plot PC3, but continue on since the plot of PC1 vs. PC2 can be informative
                    pass
        ...
        if self.legendPos != -1:
            ...
            groupNames = []
            for i in range(0, len(profile.activeSamplesInGroups)):
                legendItem = Rectangle((0, 0), 1, 1,
                                       fc=str(self.preferences['Group colours'][profile.activeGroupNames[i]].name()))
    ...
    def configure(self, profile, statsResults):
        ...
        if configDlg.exec_() == QtWidgets.QDialog.Accepted:
            ...
            self.settings.setValue('multiple group: ' + self.name + '/fixed pixels per unit distance',
