...
class ScatterPlot(AbstractGroupPlotPlugin):
    ...
    def __init__(self, preferences, parent=None):
        super(ScatterPlot, self).__init__(preferences, parent)
    ...
    def mirrorProperties(self, plotToCopy):
        super(ScatterPlot, self).mirrorProperties(plotToCopy)
        ...
        self.bShowR2 = plotToCopy.bShowR2
    ...
    def plot(self, profile, statsResults):
        ...
        if len(profile.profileDict) > 10000:
            ...
            reply = QtWidgets.QMessageBox.question(self, 'Continue?',
                                                   'Profile contains ' + str(len(profile.profileDict)) + ' features. ' +
        ...
        # --- FIX: Convert dict_keys to list for indexing ---
        features = list(profile.getFeatures())
        # ---------------------------------------------------
        ...
        axesScatter = self.fig.add_axes([xOffsetFigSpace, yOffsetFigSpace,
                                         1.0 - xOffsetFigSpace - histogramSizeX - (2 * padding) / self.figWidth,
                                         1.0 - yOffsetFigSpace - histogramSizeY - (2 * padding) / self.figHeight])
        ...
        if self.bShowHistograms:
            ...
            axesRightHistogram = self.fig.add_axes([1.0 - histogramSizeX - padding / self.figWidth, yOffsetFigSpace,
                                                    histogramSizeX, 1.0 - yOffsetFigSpace - histogramSizeY - (
                                                                2 * padding) / self.figHeight])
        ...
        for i in range(0, len(field1)):
            ...
            if statsResults.profile is not None:
        ...
        if len(highlightedField1) > 0:
            axesScatter.scatter(highlightedField1, highlightedField2, c=highlighColours, s=self.markerSize,
        ...
        # plot y=x line
        maxProportion = 1.05 * max(max(field1) if field1 else 0, max(field2) if field2 else 0)
        if maxProportion == 0: maxProportion = 1.0

        axesScatter.plot([0, maxProportion], [0, maxProportion], color=axesColour, linestyle='dashed', marker='',
                         zorder=1)
        ...
        if self.bShowR2:
            axesScatter.text(0.02, 0.98, r'R$^2$ = ' + ('%0.3f' % r_value ** 2), horizontalalignment='left',
                             verticalalignment='top', transform=axesScatter.transAxes)
        ...
        if not self.bShowHistograms:
            ...
            axesTopHistogram.set_yticks([0, max(pdf) if len(pdf) > 0 else 0])
            axesTopHistogram.set_ylim([0, max(pdf) * 1.05 if len(pdf) > 0 else 1.0])

            # plot right histogram
            axesRightHistogram.yaxis.set_major_formatter(NullFormatter())
            pdf, bins, patches = axesRightHistogram.hist(field2, bins=self.numBins, orientation='horizontal',
                                                         facecolor=group2Colour)
            axesRightHistogram.set_ylim(axesScatter.get_ylim())
            axesRightHistogram.set_xticks([0, max(pdf) if len(pdf) > 0 else 0])
            axesRightHistogram.set_xlim([0, max(pdf) * 1.05 if len(pdf) > 0 else 1.0])
    ...
    def configure(self, profile, statsResults):
        ...
        if configDlg.exec_() == QtWidgets.QDialog.Accepted:
            ...
            self.spreadMethod = str(configDlg.ui.cboSpreadMethod.currentText())
