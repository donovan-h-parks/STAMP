...
class HeatmapPlot(AbstractGroupPlotPlugin):
    ...
    def __init__(self, preferences, parent=None):
        ...
        # --- FIX: Required for STAMP.py groupPlotUpdate() ---
        self.bSupportsHighlight = False
        # ----------------------------------------------------

        self.settings = preferences['Settings']

        # Explicit casting for PyQt5
        self.fieldToPlot = str(self.settings.value(self.name + '/field to plot', 'Proportion of sequences (%)'))
        self.bPlotOnlyActiveFeatures = self.settings.value(self.name + '/plot only active features', False, type=bool)
        ...
        self.bShowColDendrogram = self.settings.value(self.name + '/show col dendrogram', True, type=bool)
        self.bShowRowDendrogram = self.settings.value(self.name + '/show row dendrogram', True, type=bool)
    ...
    def plot(self, profile, statsResults):
        ...
        # Python 3: keys() returns a view, explicit list conversion required for len() or indexing
    ...
    def configure(self, profile, statsResults):
        ...
        if configDlg.exec_() == QtWidgets.QDialog.Accepted:
            ...
            self.settings.setValue(self.name + '/plot only active features', self.bPlotOnlyActiveFeatures)
            self.settings.setValue(self.name + '/width', self.figWidth)
            self.settings.setValue(self.name + '/height', self.figHeight)
            self.settings.setValue(self.name + '/sort col method', self.sortColMethod)
            self.settings.setValue(self.name + '/sort row method', self.sortRowMethod)
            self.settings.setValue(self.name + '/show col dendrogram', self.bShowColDendrogram)
            self.settings.setValue(self.name + '/show row dendrogram', self.bShowRowDendrogram)
            self.settings.setValue(self.name + '/colourmap', self.colourmap)
            self.settings.setValue(self.name + '/legend position', self.legendPos)
            self.settings.setValue(self.name + '/clustering col threshold', self.clusteringColThreshold)
            self.settings.setValue(self.name + '/clustering row threshold', self.clusteringRowThreshold)
            self.settings.setValue(self.name + '/dendrogram col height', self.dendrogramHeight)
            self.settings.setValue(self.name + '/dendrogram row width', self.dendrogramWidth)
