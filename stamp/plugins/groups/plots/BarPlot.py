...
class BarPlot(AbstractGroupPlotPlugin):
    ...
    def plot(self, profile, statsResults):
        ...
        rects2 = axesBar.bar(np.arange(len(data2)) * colWidth + len(data1) * colWidth, data2, width=barWidth,
                             color=group2Colour, zorder=5)
        ...
        if self.bShowAverages:
            ...
            axesBar.plot([-0.25 * colWidth, len(data1) * colWidth - 0.25 * colWidth], [avgGroup1, avgGroup1],
                         color=group1Colour, linestyle='-', zorder=1)
            axesBar.plot([len(data1) * colWidth - 0.25 * colWidth, len(sampleNames) * colWidth - 0.25 * colWidth],
                         [avgGroup2, avgGroup2], color=group2Colour, linestyle='-', zorder=1)
        ...
        if self.bShowPvalue and statsResults.profile is not None:
            ...
            axesBar.text(1.0, 1.0, r'$p$ = ' + pValueStr, horizontalalignment='right', verticalalignment='bottom',
                         transform=axesBar.transAxes)
        ...
        if self.legendPos != -1:
            legend = axesBar.legend([rects1[0], rects2[0]], (profile.groupName1, profile.groupName2),
                                    loc=self.legendPos)
