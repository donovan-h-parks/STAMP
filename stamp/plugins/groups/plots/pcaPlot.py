...
class pcaPlot(AbstractGroupPlotPlugin):
    ...
    def __init__(self, preferences, parent=None):
        super(pcaPlot, self).__init__(preferences, parent)
        ...
        # --- FIXES FOR ATTRIBUTE ERRORS ---
        self.bRunPostHocTest = False  # Required for MultiGroup compatibility
        self.bSupportsHighlight = False  # Required for GroupPlotUpdate in STAMP.py
        # ----------------------------------

        self.settings = preferences['Settings']

        # PyQt5 settings return native types; explicit casting prevents type errors
        ...
        self.bFixedPixelsPerUnitDistance = self.settings.value(
            'group: ' + self.name + '/fixed pixels per unit distance', True, type=bool)
        self.markerSize = int(self.settings.value('group: ' + self.name + '/marker size', 30))
        self.bRotateLabels = self.settings.value('group: ' + self.name + '/rotate pc3 labels', True, type=bool)
        self.bShowPC1vsPC3 = self.settings.value('group: ' + self.name + '/showPC1vsPC3', True, type=bool)
        self.bShowPC3vsPC2 = self.settings.value('group: ' + self.name + '/showPC3vsPC2', True, type=bool)
        self.legendPos = int(self.settings.value('group: ' + self.name + '/legend position', -1))
        self.bUniqueShapes = self.settings.value('group: ' + self.name + '/unique shapes', True, type=bool)
    ...
    def mirrorProperties(self, plotToCopy):
        ...
        # Copy flags if they exist in source
    ...
    def plot(self, profile, statsResults):
        ...
        try:
            ...
            self.emptyAxis(
                'Linear algebra error likely due to a singular matrix (i.e., degenerate data).\nTry plotting at a different level in the hierarchy.')
            return
        except Exception:
        ...
        if (bPlot2D == False) and self.bShowPC1vsPC3 and self.bShowPC3vsPC2:  # all three plots
            if self.bFixedPixelsPerUnitDistance:
                pc1Percentage = (max(pc1) * axesExpandPercentage - min(pc1) * axesExpandPercentage) / (
                            (max(pc1) * axesExpandPercentage - min(pc1) * axesExpandPercentage) + (
                                max(pc3) * axesExpandPercentage - min(pc3) * axesExpandPercentage))
                ...
                pc2AxisLengthY = pc1AxisLengthX * (
                            (max(pc2) * axesExpandPercentage - min(pc2) * axesExpandPercentage) / (
                                max(pc1) * axesExpandPercentage - min(pc1) * axesExpandPercentage))
                pc3AxisLengthY = pc1AxisLengthX * (
                            (max(pc3) * axesExpandPercentage - min(pc3) * axesExpandPercentage) / (
                                max(pc1) * axesExpandPercentage - min(pc1) * axesExpandPercentage))
            ...
            if math.isnan(figHeight) or math.isnan(pc1AxisLengthX) or math.isnan(pc2AxisLengthY) or (
                    pc2AxisLengthY == 0):
                self.emptyAxis('Degenerate PCA plot')
                return
            ...
            axesScatter1 = self.fig.add_axes([xBorderFigSpace + xLabelOffsetFigSpace,
                                              yBorderFigSpace + yLabelOffsetFigSpace + pc3AxisLengthY / figHeight + yPlotSpacingFigSpace,
                                              pc1AxisLengthX / self.figWidth, pc2AxisLengthY / figHeight])
            axesScatter2 = self.fig.add_axes(
                [xBorderFigSpace + xLabelOffsetFigSpace + pc1AxisLengthX / self.figWidth + xPlotSpacingFigSpace,
                 yBorderFigSpace + yLabelOffsetFigSpace + pc3AxisLengthY / figHeight + yPlotSpacingFigSpace,
                 pc3AxisLengthX / self.figWidth, pc2AxisLengthY / figHeight])
            axesScatter3 = self.fig.add_axes(
                [xBorderFigSpace + xLabelOffsetFigSpace, yBorderFigSpace + yLabelOffsetFigSpace,
            ...
            axesScatter1 = self.fig.add_axes(
                [xBorderFigSpace + xLabelOffsetFigSpace, yBorderFigSpace + yLabelOffsetFigSpace,
                 pc1AxisLengthX / self.figWidth, pc2AxisLengthY / figHeight])
            axesScatter2 = self.fig.add_axes(
                [xBorderFigSpace + xLabelOffsetFigSpace + pc1AxisLengthX / self.figWidth + xPlotSpacingFigSpace,
                 yBorderFigSpace + yLabelOffsetFigSpace,
                 pc3AxisLengthX / self.figWidth, pc2AxisLengthY / figHeight])
        ...
        axesScatter1.scatter(pc1[0:numGroup1Samples], pc2[0:numGroup1Samples], s=self.markerSize, zorder=5,
                             c=colours[0:numGroup1Samples], marker=markers[0])
        axesScatter1.scatter(pc1[numGroup1Samples:], pc2[numGroup1Samples:], s=self.markerSize, zorder=5,
                             c=colours[numGroup1Samples:], marker=markers[1])
        ...
        if (bPlot2D == False) and self.bShowPC3vsPC2:
            try:
                axesScatter2.scatter(pc3[0:numGroup1Samples], pc2[0:numGroup1Samples], s=self.markerSize, zorder=5,
                                     c=colours[0:numGroup1Samples], marker=markers[0])
                axesScatter2.scatter(pc3[numGroup1Samples:], pc2[numGroup1Samples:], s=self.markerSize, zorder=5,
                                     c=colours[numGroup1Samples:], marker=markers[1])
            except Exception:
        ...
        if (bPlot2D == False) and self.bShowPC1vsPC3:
            try:
                axesScatter3.scatter(pc1[0:numGroup1Samples], pc3[0:numGroup1Samples], s=self.markerSize, zorder=5,
                                     c=colours[0:numGroup1Samples], marker=markers[0])
                axesScatter3.scatter(pc1[numGroup1Samples:], pc3[numGroup1Samples:], s=self.markerSize, zorder=5,
                                     c=colours[numGroup1Samples:], marker=markers[1])
            except Exception:
                # failed to plot PC3, but continue on since the plot of PC1 vs. PC2 can be informative
                pass
        ...
        if self.legendPos != -1:
            ...
            legend = self.fig.legend([legend1, legend2], (profile.groupName1, profile.groupName2), loc=self.legendPos,
                                     ncol=2)
    ...
    def configure(self, profile, statsResults):
        ...
        if configDlg.exec_() == QtWidgets.QDialog.Accepted:
            ...
            # legend position
            if configDlg.ui.radioLegendPosUpperLeft.isChecked():
                self.legendPos = 2
            elif configDlg.ui.radioLegendPosLowerLeft.isChecked():
                self.legendPos = 3
            elif configDlg.ui.radioLegendPosUpperCentre.isChecked():
                self.legendPos = 9
            ...
            self.settings.setValue('group: ' + self.name + '/fixed pixels per unit distance',
