...
class White(AbstractGroupStatsTestPlugin):
    ...
    def __init__(self, preferences):
        super(White, self).__init__(preferences)
    ...
    def runAll(self, seqGroup1, seqGroup2, parentSeqGroup1, parentSeqGroup2, confIntervMethod, coverage, progress):
        return detect_differentially_abundant_features(seqGroup1, seqGroup2, parentSeqGroup1, parentSeqGroup2, coverage,
                                                       self.preferences['Replicates'], self.preferences, progress)
...
def detect_differentially_abundant_features(seqGroup1, seqGroup2, parentSeqGroup1, parentSeqGroup2, coverage, B,
    ...
    pValuesOneSided, pValuesTwoSided, lowerCIs, upperCIs = permuted_statistics(propGroup1, propGroup2, seqGroup1,
                                                                               seqGroup2, T_statistics, coverage, B,
                                                                               progress)
    ...
    for r in range(0, numFeatures):
        if sum(seqGroup1[r]) < n1 and sum(seqGroup2[r]) < n2:
            p1, p2, note = fishers.hypothesisTest(sum(seqGroup1[r]), sum(seqGroup2[r]), sum(parentSeqGroup1[r]),
                                                  sum(parentSeqGroup2[r]))
            l, u, es, note = diffBetweenProp.run(sum(seqGroup1[r]), sum(seqGroup2[r]), sum(parentSeqGroup1[r]),
                                                 sum(parentSeqGroup2[r]), coverage)
...
def permuted_statistics(propGroup1, propGroup2, seqGroup1, seqGroup2, T_statistics, coverage, B, progress):
    ...
    if progress is not None and progress != 'Verbose':
    ...
    permuted_ttests = []
    # permuted_effectSizes = [] # Unused variable
    for j in range(0, B):
        if progress is not None and progress != 'Verbose':
            progressIndex += 1
        ...
        # Python 3: xrange replaced by range
        trial_ts, trail_effectSizes, notes = permute_and_calc_ts(propGroup1, propGroup2,
                                                                 random.sample(range(numSamples), numSamples))
    ...
    if n1 < 8 or n2 < 8:
        ...
        if progress is not None:
            progress.setMaximum(len(highFreqIndices))  # Fixed: len(highFreqIndices) instead of list itself

        # now for each feature
        pValuesOneSided = [0.0] * numFeatures
        pValuesTwoSided = [0.0] * numFeatures
        # lowerCIs and upperCIs are recalculated later, initialized here
        ...
        # print('blah') # debug print removed
        if progress is not None:
